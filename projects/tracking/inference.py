...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        pNoisyGivenTrueDist = busters.getObservationProbability(noisyDistance, trueDist)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for pos in self.allPositions:
            pObs = self.getObservationProb(observation, pmPos, pos, self.getJailPosition())
            pGhostPos = self.beliefs[pos]
            self.beliefs[pos] = pObs * pGhostPos
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.allPositions:
            newBeliefs[pos] = 0
        for pos in self.allPositions:
            newGhostPosDist = self.getPositionDistribution(gameState, pos)
            for newPos, transition in newGhostPosDist.items():
                newBeliefs[newPos] += self.beliefs[pos] * transition
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        particlesAtEach = self.numParticles / len(legalPos)
        for pos in legalPos:
            for _ in range(int(particlesAtEach)):
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        legalPos = self.legalPositions
        self.particles = list(itertools.product(legalPos, repeat = self.numGhosts))
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for perm in permutations:
            ...
            for i in range(self.numGhosts):
                obsProb *= self.getObservationProb(observation[i], pmPos, perm[i], self.getJailPosition(i))
            distr[perm] = obsProb * beliefDistr[perm]
