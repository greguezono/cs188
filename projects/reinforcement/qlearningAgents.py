...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        if state not in self.values.keys():
            ...
            if not self.values[state]:
                return 0.0
            if action not in self.values[state].keys():
                return 0.0
    ...
    def update(self, state, action, nextState, reward):
        ...
        if (not state in self.values.keys() or isinstance(self.values[state], int)):
            self.values[state] = {}
